...
export const Navbar: FC = () => {
  ...
  const toggleSideMenu = () => {
    if (sideMenuRef.current) {
      ...
      sideMenuRef.current.classList.toggle("translate-x-72");
    }
  };
  ...
  return (
    <div className="flex justify-center w-full">
      ...
      <nav className="w-full xl:w-[1280px] fixed px-5 lg:px-8 py-4 flex items-center justify-between z-50">
        <Link href="#top" className="w-52 flex justify-center">
        ...
        <div className="w-52 flex justify-between items-center gap-4">
          <button>
            <Image src={icon.moon} alt="icon-moon" className="w-6" />
          </button>
          <a
            href="#contact"
            className="hidden lg:flex items-center gap-3 px-10 py-3 border border-gray-400 rounded-full ml-4">
            Connect <Image src={icon.plugZap} alt="plug-connect" />
          </a>
          <button onClick={toggleSideMenu} className="block md:hidden ml-3">
            <Image src={icon.alignRight} alt="icon-menu" className="w-6" />
          </button>
        </div>
        ...
        <ul
          ref={sideMenuRef}
          className="flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-0 top-0 bottom-0 w-72 z-50 h-screen bg-rose-50 transition duration-500">
